...
async function toggleStamp(vendorId) {
  ...
  if (nowDone) {
    const btn = document.querySelector(`.stamp-btn[data-id="${vendorId}"]`);
    if (btn) {
      btn.classList.add('just-stamped');
      setTimeout(() => btn.classList.remove('just-stamped'), 450);
    }
  }
...
async function handleAddVendorSubmit(e) {
  ...
  try {
    ...
    state.loaded.today = false; // Today tab will pick up the new vendor next time it's shown
    closeModal();
...
async function openVendorDetail(vendorId) {
  state.returnView = state.view === 'vendorDetail' ? (state.returnView || 'today') : state.view;
...
const handlers = {
  ...
  'back': () => switchView(state.returnView || 'today'),
